...
export class YouTubeSearchService {
  ...
  search(query: string): Observable<SearchResult[]> {
    ...
    var getObs: Observable<any> = this.http.get(queryUrl);
    var responseObs: Observable<any> = getObs.map((res:any) => <any>res['items']);
   var itemObs: Observable<any> = responseObs.map(item => {
      console.log(">>>> single item:", item);
      return item;
    });

    responseObs.subscribe((item: any) => {
      item.map(item => {
        console.log('single item: ', item)
      });
    });
    ...
    return this.http.get(queryUrl).map((res: any) => {
